...
describe('editor body document', () => {
  it('round-trips ordered text, whitespace, newlines and atomic links', () => {
    const body: BodySegment[] = [
      ...
      {
        type: 'link',
        targetCardId: parseCardId('01991f20-61d2-7000-8000-000000000001'),
      },
      { type: 'text', text: '\n 後' },
    ];
    expect(editorDocumentToSegments(segmentsToEditorDocument(body))).toEqual(
      body,
    );
  });
  ...
  it('does not infer links from hashtag-like plain text', () => {
    const body: BodySegment[] = [
      {
        type: 'text',
        text: 'C# #123 ＃ URL https://example.test/#x [md](#1) 日本語',
      },
    ];
    expect(editorDocumentToSegments(segmentsToEditorDocument(body))).toEqual(
      body,
