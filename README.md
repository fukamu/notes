...
オンライン時は起動時、編集後、再接続時、および15秒間隔で同期します。同じ `mutationId` の再送は同期先で一度だけ適用されます。送信内容は応答を保存できるまで端末内に固定されるため、通信が途切れたり再読み込みしたりしても同じ内容を再送します。送信中に新しい編集が保存された場合は別の変更として保持し、応答後に続けて送信するため、古い応答で本文を巻き戻しません。
