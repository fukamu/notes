...
Issue、統合／作業ブランチ、PR、merge後検証、型付き純粋ロジックと副作用adapter、mainへの反映制限は [Issue-based development workflow](docs/development-workflow.md) を正本とします。実装IssueのPRは統合ブランチだけをbaseとし、利用者が対象を特定して明示的に許可するまでmainへ変更を反映しません。
