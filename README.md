...
保持する関係は本文に書かれた `A → B` だけです。「つながり」は端末内の全カード（孤立カードと非連結コンポーネントを含む）を表示し、全カードの本文から、存在するリンク先への明示的な一方向リンクを都度列挙します。同じ有向ペアが本文に複数あっても表示上は1辺へ集約し、欠損したリンク先は無視します。現在カードは強調表示にだけ使い、表示対象の絞り込みには使いません。逆向きの関係、被リンク件数、被リンク用のAPI・型・保存データはありません。

配置は端末内の専用Workerが全カードと全リンクを数値配列として処理し、孤立カードと非連結コンポーネントを含む決定的な全体配置を作ります。全体像は全要素を保持するWebGL2キャンバス（利用できない場合は段階構築するCanvas2D）で描画し、拡大時だけ表示範囲内のカード形状、タイトル、方向表示、操作対象を追加します。検索や枚数上限で表示対象を減らすことはありません。「全体」で俯瞰へ戻り、「現在地」で現在カードを中央へ移せます。配置計算と表示はCDNやネットワークへ依存しません。
...
- 全体表示では画面の画素数よりカードやリンクが多い場合に重なって密度として見えます。拡大すると個々のカードとリンクを確認できます。
- 任意の有向グラフでは交差を常にゼロにできません。配置とセル間経路で長い配線やカードへの侵入を抑えますが、密グラフでは線が多くなります。
