...
| 設定                           | 対象runtime                       | 主な対象                                        |
| ------------------------------ | --------------------------------- | ----------------------------------------------- |
| `tsconfig.json`                | browser / React                   | `app`、`components`、`frontend`、`hooks`、`lib` |
| `tsconfig.service-worker.json` | Service Worker                    | `public/sw.js`（`checkJs`）                     |
| `tsconfig.tooling.json`        | Node.js                           | Vite、Vitest、Playwright、検証script            |
| `tsconfig.test.json`           | Node.js + 明示したbrowser fixture | unit / integration / E2E test                   |

`skipLibCheck: true` は、ViteやReactなど複数の第三者宣言の検査に限定して残しています。上記すべてのentryでアプリの `.ts`、`.tsx` と対象の `.js` は通常どおり検査されるため、自コードの検査を除外する設定ではありません。

Oxlintはbrowser app、Service Worker、tooling、testの全対象でunsafe assignment／argument／call／member access／return、不要なassertion、non-null assertion、switch exhaustivenessをerrorにします。Go serverは`go vet`、unit／integration／race test、buildに加え、Go AST architecture testでdomain packageから`adapters`、`httpapi`、`runtimefoundation`、`cmd`への逆依存を禁止します。Service Workerは `service-worker/sw.ts` を型付き正本とし、build時に静的asset `public/sw.js` を生成してproduction成果物も検査します。
...
## Go API／PostgreSQL境界とatomicity

Go HTTP boundaryはbody byte上限を確認し、strict JSON decode後にUUIDv7、整数範囲、timestamp順序、本文、件数、重複、unknown field、mutation kind別invariantを検査します。違反は400、payload超過は413です。request pathではschema DDLを実行せず、未migration環境はfail closedにします。内部障害はgenericな5xxとし、responseとlogにはraw request、row、secretを含めません。

PostgreSQL adapterはscanしたnullable columnとJSON bytesを明示的に検査し、型付きrecordへ変換してからapplicationへ渡します。rowからmappingしたserver outputもprotocol invariantを再検査してから返します。

`resolve` は非空・重複なしのconflict IDに加え、全IDが対象cardとVaultに属することとbase revisionをtransaction内で確認します。card CAS、対象conflict削除、journal receipt、quota確定はPostgreSQL transactionでatomicに確定し、条件不成立や途中失敗はrollbackします。このため確認後race、別card／別Vault／存在しないID、stale revisionで上書きできません。

## schemaとenvironment

`backend/migrations`のversioned PostgreSQL migrationを正本とし、migration tableと期待versionを起動前readinessで照合します。DDLは`notesctl migrate`の明示runnerだけが実行し、request handlerやruntime初期化はDDLを実行しません。旧Drizzle/D1資料は歴史資料であり、現行schemaや運用権限ではありません。

request-time runtimeはGoとPostgreSQLだけです。旧TypeScript/D1 source、test、config、dependencyはT17で撤去済みです。Goはenvironmentを起動時にdecodeし、PostgreSQL rowをscan後に検査し、欠落・不正設定や未migration schemaをfail closedにします。`npm run check:environment` は静的frontend route、public build allowlist、Go起動、container assetの整合を照合します。
...
| Go API request                | byte上限、strict JSON、typed validation、400／413分類                 | T05–T17       |
| PostgreSQL row / JSON column  | scan後にnullable性、ID、時刻、JSON shapeを検査                        | T05–T17       |
...
Phase 1〜3でcompiler、codec／brand、client／IndexedDB境界を整備し、T05–T17でserver境界をGo／PostgreSQLへ移しました。親 #9 の要件1–30の証跡は `docs/type-safety-audit.md` に記録します。
...
- schema変更なら、PostgreSQL migration、期待version、rollback手順を更新したか。

## invalid dataの回復方針

不正なnetwork／API inputは破棄し、端末側pending mutationを残して再試行可能にします。不正なIndexedDB recordは自動削除・上書きせず、利用者向け失敗表示と診断を残します。不正なPostgreSQL row／JSONは成功responseとして返さず停止し、該当requestによる追加writeを行いません。修復は検証済みbackupや管理手順による明示操作とし、request処理中に推測修復しません。

## 共通の検証入口

`npm run verify` がlocalとCIの共通入口です。format check、browser/build-time TypeScript typecheck、全面unsafe lint、Vitest、Go unit／PostgreSQL integration／race／architecture test、production build、Desktop ChromeとPixel 7相当のE2E、Go-only runtime image検証を順に実行します。

個別調査には `npm run test:integration`、`npm run test:architecture`、`npm run check:environment` を使えます。CIとtestはbrowser fixtureと専用PostgreSQL test databaseだけを使い、本番database、本番データ、デプロイを使用しません。

## Pure coreとIssue実行管理

業務判断、変換、状態遷移はtyped pure coreへ置き、React、clock、UUID、network、IndexedDB、PostgreSQL、DOM等はadapterから検証・生成済み値を渡します。TypeScriptとGo双方のarchitecture testがcore→concrete effectの逆依存を機械検査し、意味論的な純粋性と入力非変更はunit test/reviewで補います。
...
Go server content pathはverified sessionから導出した`identity.VaultContext`をSync v2、encrypted-object、PostgreSQL adapterへ渡します。repository commandはrequest bodyからAccountId/VaultIdを受け取らず、PostgreSQLのread/write/CASはAccount/Vault scopeと期待revisionをpredicateへ含めます。cross-tenant、stale revision、owner mismatchはopaqueな拒否としてfail closedになり、scanしたrowはdomain invariantを再検査してから内側へ渡します。[Vault-scoped server repository and tenant routing](vault-content-repository.md) のD1設計は移行元の歴史資料であり、現行実装の正本は`backend/internal/syncv2`、`backend/internal/encryptedobject`、`backend/internal/adapters/postgres`です。
...
Vault DEK rotationはgenerating/promoting/completedとCAS revisionをGoのpure state machineで管理し、PostgreSQL/KMSをservice adapterの外側に置きます。PostgreSQL repositoryはAccountId/VaultId ownerを全load/mutationで固定し、wrapped metadataの永続化後だけoperation rowをcompletedへ進めます。pending中はsource、completed後だけtargetをlogical write versionとし、全旧versionをmixed read用に保持します。raw DEK handleは生成結果の検証後に必ずdestroyし、old keyの削除やproduction KMS操作は行いません。詳細は [Vault DEK rotation lifecycle](dek-rotation.md) を参照してください。

既存暗号文の再暗号化は、昇格済みwrite DEKと結びついたtyped checkpointでVault内を最大100件ずつ走査します。旧暗号文を記録済みAADで認証してからfresh ciphertextをimmutable保存し、logical revisionを変えずにmetadata CASと旧object delete outboxを同一PostgreSQL transactionで確定します。CAS競合、途中失敗、checkpointより前へ追加された旧version、旧DEKのpending writeを明示的な再試行状態にし、対象なしではobject storageやKMSを呼びません。詳細は [Vault DEK rotation lifecycle](dek-rotation.md#existing-ciphertext-re-encryption) を参照してください。

DEK recovery drillはversioned backup manifestとciphertextをGoのstrict JSON boundaryからdecodeし、移行時に固定した共有fixture上のmixed旧/新versionを正確なVault/object/revision AADで認証します。plaintext/raw keyをreceiptへ含めず、incomplete checkpoint、missing/wrong key、swap、retention違反をblocked resultにします。retirement pure gateはactive/pending/backup/drill evidenceが揃ってもdeleteを返さず、別のproduction key destruction承認が必要なterminal stateで停止します。Goのbackup portはcopying in-memory fixtureとlocal/test専用のread-only private-directory fixtureに限定され、production providerへ接続しません。詳細は [DEK rotation recovery drill and retirement gate](dek-rotation-recovery.md) を参照してください。

Personal Vault quotaは表示文字、serialized plaintext bytes、ciphertext bytes、request bytes、Vault usageを別のrefined measureとして扱います。表示文字は正規化しないUnicode scalar valueで数え、unpaired surrogateを拒否します。pure transitionはcreate/update/delete deltaとexact limitを決め、scope-bound PostgreSQL ledgerがpositive reservationとserializable transaction／CASで並行上限を守ります。expired reservationは自動解放せず、content/journal側のdurable evidenceに基づく明示finalizeだけを許可します。authenticated Sync v2はEntitlementから得たlimitと実測値をこのledgerへ接続し、reservation、encrypted write、journal receipt、quota finalizeの順で確定します。詳細は [Personal Vault quota policy](quota-policy.md)、[Vault quota ledger](quota-ledger.md)、[Sync v2 quota enforcement](sync-v2-quota.md) を参照してください。

Google OIDC境界はstate、nonce、PKCE verifier/challenge、authorization code、issuer、subject、client ID、redirect URIを別brandで表します。start/callback、provider verified claims、pending transaction、identity directoryの値はすべて`unknown`からdecodeし、exact redirect/issuer/audience、`azp`、expiry/issued-at、nonceをpure coreで判定します。transaction storeはstateを原子的にconsumeし、同じcallbackを再利用できません。emailはverified attributeであってidentity keyではなく、既存accountへのlink対象はrequest bodyではなくauthenticated `VaultContext`からだけ導出します。provider通信・signature/JWKS検証・entropy・clock・transaction/identity storageはGoのport/adapter側に留めます。旧TypeScriptのWeb Crypto adapterはT17で撤去済みで、現行server契約としては実行されません。
