...
- deciding commit versus release;
- finalizing or automatically releasing a reservation;
...
Stop invoking the command and roll back the application artifact to the prior
integration commit. T13a adds no schema and writes no data, so there is no data
rollback. Preserve emitted audit evidence for review; do not delete or finalize
the underlying reservations as part of application rollback.
